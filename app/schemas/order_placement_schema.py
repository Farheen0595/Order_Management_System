...
class OrderPlacementInput(BaseModel):
    ...
    """
    Schema for creating an order, updating order audit, and adjusting inventory.
    
    Fields:
        product_id: Unique numeric ID of the product from Inventory table.
        quantity: Number of units to purchase. Must be positive and <= available stock.
        customer_email: Email address for order confirmation.
        remarks: Optional notes about the order (default: 'Order placed').
    """

    product_id: int = Field(..., description=("Unique numeric identifier of the product from the Inventory table "
                                                "(NOT the SKU). Must match a valid product_id in the Inventory."), gt=0)

    quantity: int = Field(..., gt=0, description=("Number of units the customer wants to purchase. Must be a positive integer "
                                                 "and should not exceed available inventory."))

    customer_email: EmailStr = Field(default="aiagent05@gmail.com", description="Customer's email address for order confirmation. Must be a valid email format.")

    remarks: str = Field( default="Order placed", description="Optional notes about the order, e.g., 'Customer order'.")
